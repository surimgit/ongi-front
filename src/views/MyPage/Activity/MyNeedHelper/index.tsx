...
import {
  accpetChatRequest,
  accpetHelperApplyRequest,
  getApplicantCountRequest,
  getHelperApplyListRequest,
  getHelperCommentsRequest,
  getHelperApplyRequest,
  getMyHelperApplyPostRequest,
  getMyHelperlikedPostRequest,
  getMyHelperRequestPostRequest,
} from "src/apis";
  ...
  return (
    <div className="tr">
      ...
      {type === "apply" ? (
        ...
        <div
          className="td applicants-box"
          onClick={() => onApplyModalOpen(sequence)}
        >
          {applicantCount}
...
export default function MyNeedHelper({ type }: Props) {
  ...
  // state: 신청 수락 여부 상태 //
  const [appliedStatusMap, setAppliedStatusMap] = useState<{
    [key: number]: boolean;
  }>({});
  ...
  const onApplyModalOpenButtonClickHanlder = (postSequence: number) => {
    ...
    if (openState) {
      getHelperApplyListRequest(postSequence, accessToken).then(
        getHelperApplyListResponse
      );
    }
  };
  ...
  const onAcceptApplicantsClickHandler = (applicantId: string) => {
    ...
    const applicant = applicants?.find(
      (apply) => apply.applicantId === applicantId
    );
    ...
    accpetHelperApplyRequest(postSequence, applicantId, accessToken).then(
      (responseBody) =>
        accpetHelperApplyResponse(
          responseBody,
          postSequence,
          applicantId,
          chatSequence
  ...
  const accpetHelperApplyResponse = async (
    ...
    const chatAcceptResponse = await accpetChatRequest(
      chatSequence,
      accessToken,
      applicantId
    );
  ...
  const getHelperApplyListResponse = async (
    responseBody: GetHelperApplyListResponseDto | ResponseDto | null
  ) => {
  ...
  const fetchPostDetails = async (
    post: MyNeedHelperPost
  ): Promise<MyNeedHelperPost> => {
    ...
    const applicantCount =
      typeof applicantCountResponse === "number" ? applicantCountResponse : 0;
    const commentCount =
      commentsResponse && "comments" in commentsResponse
  ...
  const getMyHelperRequestPostResponse = async (
    responseBody: GetMyHelperPostResponseDto | ResponseDto | null
  ) => {
    const message = !responseBody
      ? "서버에 문제가 있습니다."
      : responseBody.code === "DBE"
  ...
  const getMyHelperApplyPostResponse = async (
    responseBody: GetMyHelperPostResponseDto | ResponseDto | null
  ) => {
    const message = !responseBody
      ? "서버에 문제가 있습니다."
  ...
  const getMyHelperlikedPostResponse = async (
    responseBody: GetMyHelperPostResponseDto | ResponseDto | null
  ) => {
    const message = !responseBody
      ? "서버에 문제가 있습니다."
  ...
  useEffect(() => {
    ...
    const changeType = async () => {
      if (type === "ask") {
        ...
        getMyHelperRequestPostRequest(accessToken).then(
          getMyHelperRequestPostResponse
        );
      }

      if (type === "apply") {
        setTitle("내가 신청한 도우미 게시글");
        getMyHelperApplyPostRequest(accessToken).then(
          getMyHelperApplyPostResponse
        );
      }

      if (type === "liked") {
        setTitle("내가 좋아요한 도우미 게시글");
        getMyHelperlikedPostRequest(accessToken).then(
          getMyHelperlikedPostResponse
        );
      }
    };
    changeType();
  ...
  useEffect(() => {
    ...
    const fetchAllIsApplied = async () => {
      ...
      for (const post of viewList) {
        try {
          const response = await getHelperApplyRequest(
            post.sequence,
            accessToken
          );
          if (response && response.code === "SU") {
            results[post.sequence] = (
              response as GetHelperIsApplyResponseDto
            ).isApplied;
          }
        } catch (error) {
      ...
      <div className="contents-wrapper">
        ...
        <div className="need-helper-list">
          ...
          {isApplyModalOpen && applicants && type !== "liked" && (
            <Modal title="신청자 목록" onClose={closeApplyModal}>
              <div className="modal-applicants-wrapper">
                ...
                <div className="applicants-list">
                  {applicants.length === 0 ? (
                    <div className="applicants-notice">
                      아직 신청자가 없습니다!
                    </div>
                  ) : (
                    applicants.map((applicant, index) => (
                      <div className="applicant-row" key={index}>
                        ...
                        <div className="requested-at">
                          {new Date(applicant.requestedAt).toLocaleString()}
                        </div>
                        <div className="is-applied">
                          {applicant.isApplied ? "승인됨" : "대기중"}
                        </div>
                        <div className="action">
                          {!applicant.isApplied ? (
                            <div
                              ...
                              onClick={() =>
                                onAcceptApplicantsClickHandler(
                                  applicant.applicantId
                                )
                              }
                            >
