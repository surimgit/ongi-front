...
export default function Activity() {
  ...
  const [communityPostCount, setcommunityPostCount] = useState<number | string>('0');
  const [reviewCount, setReviewCount] = useState<number | string>('0');
  const [reviewedCount, setReviewedCount] = useState<number | string>('0');
  const [shoppingCartCount, setShoppingCartCount] = useState<number | string>('0');
  const [wishListCount, setWishListCount] = useState<number | string>('0');
  ...
  const getMyActivityCountResponse = (responseBody:GetMyActivityCountResponseDto | ResponseDto | null) => {
    const message = 
    ...
    const {communityCommentCount, communityPostCount, reviewCount, reviewedCount, shoppingCartCount, wishListCount} = responseBody as GetMyActivityCountResponseDto;
          ...
          <div className='my-container'>
            ...
            <div className='category-container'>
              <div className='category-box'>
                ...
                <div className='category-number' onClick={onReviewClickHandler}>{reviewCount}개</div>
              </div>
              <div className='category-box'>
                <div className='category'>• 도움 받은 후기</div>
                <div className='category-number' onClick={onReviewedClickHandler}>{reviewedCount}개</div>
              </div>
            </div>
          </div>
          <div className='my-container'>
          ...
          <div className='my-container'>
            ...
            <div className='category-container'>
              <div className='category-box'>
                ...
                <div className='category-number' onClick={onCommunityPostClickHandler}>{communityPostCount}개</div>
              </div>
              <div className='category-box'>
                <div className='category'>• 내 댓글</div>
                <div className='category-number' onClick={onCommunityCommnetClickHandler}>{communityCommentCount}개</div>
              </div>
            </div>
          </div>
        </div>
